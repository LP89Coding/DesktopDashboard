...
using DIStickyNotes.Views;
using DIStickyNotes.Internals;
using EventID = DIStickyNotes.Internals.EventID.DIStickyNotes;

namespace DIStickyNotes.Common
{
    public class Plugin : IPlugin
    {
        ...
        private void Initialize()
        {
            ...
            {
                sw.Start();
                Logger.Log(EventID.Application.InitializeStickyNotesEnter);
            ...
            {
                Logger.Log(EventID.Application.Exception, "DIStickyNotesPluginInitialize", ex);
            }
            finally
            {
                sw.Stop();
                Logger.Log(EventID.Application.InitializeStickyNotesExit, sw.ElapsedMilliseconds);
            }
        }
        #endregion
        ...
        private IWindowControl InitializeControl()
        {
            ...
            this.control = new ucStickyNotes();
            return this.control;
        }
        #endregion
        ...
        private void Close()
        {
            ...
            try
            {
                ...
                {
                    Logger.Log(EventID.Application.Exception, "DIStickyNotesPluginCloseMainWindow", ex);
                }
                try
                {
                    control?.Dispose();
                ...
                {
                    Logger.Log(EventID.Application.Exception, "DIStickyNotesPluginCloseControl", ex);
                }
                if (!this.isPluginMode)
                {
            ...
            {
                if (this.isPluginMode)
                    Logger.Log(EventID.Application.Exception, "DIStickyNotesPluginClose", ex);
                else
        ...
        {
            return ResourceImage.SmallAddressBook;
        }

        public System.Drawing.Bitmap GetLargeImage()
        {
            return ResourceImage.LargeAddressBook;
        }
